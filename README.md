...
难点在于：麦克风同时听得到自己的扬声器。这里不去做回声消除（太重），
而是拿**正在播出去的电平**当参考，估一个回声泄漏系数（能量比）：

```
预计的回声能量 = 泄漏系数 × 参考能量
只有麦克风能量明显高于这个值，才算「你在说话」
```

几个让它靠得住的细节：

- 两边都按同一个 0.3 秒滑窗算能量，顺便吸收了扬声器→麦克风的几十毫秒延迟
- 第一次播放先学 0.5 秒回声再开始判断（初值只能猜，学一下才准）
- 泄漏系数**降得快、升得慢**：估高了会漏掉真插话所以赶紧降；
  估低了慢慢抬，免得你自己说话反而把门槛顶上去
...
| `margin` | 比回声响几倍才算插话，默认 1.6；调大更稳、调小更灵敏 |
| `min_rms` | 绝对下限，压住底噪 |
| `leak_init` | 回声泄漏初值（能量比） |

想量一下这台机器到底漏多少回声：

```powershell
python scripts/test_bargein.py           # 合成对照：回声不该打断、插话该打断
python scripts/test_bargein.py --live    # 真机：先量回声，再让你真的插一句
```

> 本机实测：扬声器到麦克风只剩约 16% 幅度（泄漏系数 0.025），默认参数可用。

**已知局限**：扬声器音量很大、你又离麦克风很近时，回声和你说话差不多响，
物理上就分不开（开再大的 `margin` 也没用）。这种场合用耳机最省心：
泄漏系数会自动降到接近 0，任何说话都能立刻打断。
另外，**每轮回答刚开口的前 0.5 秒**还在学回声，那期间插话可能晚一两秒才生效。

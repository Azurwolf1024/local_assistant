...
│  ├─ store.py                 # JSON 存储（★跨进程文件锁★、外部改动自动重载、原子写入）
│  ├─ control.py               # ★控制台 ⇄ 服务的文件信箱（inbox/busy/done + 命令表）
│  ├─ service_ctl.py           # 服务状态/启动/停止（main.py 与控制台共用一份实现）
│  ├─ console/                 # ★可视化控制台（独立进程，见第 4 节「可视化控制台」）
│  │  ├─ app.py                #   FastAPI 应用 + 共享上下文 + 面板装载 + 静态文件
│  │  ├─ registry.py           #   面板注册表（加面板只动 panels/ 里的文件）
│  │  ├─ bus.py                #   实时通道（SSE）：日志行 + 服务状态变化
│  │  ├─ follow.py             #   后台跟随：日志尾巴（只读）
│  │  ├─ panels/               #   overview/schedule/memos/logs/voices/chat
│  │  └─ static/               #   index.html + app.js + ui.js + style.css + panels/*.js
...
python main.py ui            # ★ 可视化控制台（本地网页：日程/闹钟、备忘、日志、声线、对话）
...
```powershell
python main.py ui                  # 起控制台并自动打开浏览器（默认 http://127.0.0.1:8765）
python main.py ui --port 9000      # 换端口
python main.py ui --no-browser     # 不自动开浏览器
...
| 标签 | 能干什么 |
| --- | --- |
| **概览** | 服务在不在跑（PID、最后一次写日志是什么时候）、当前角色与声线、今天还剩哪些提醒、技术配置一览；右上角一键**启动 / 停止服务**，还能「念一句试听」「弹个提醒」「问服务状态」 |
| **日程 / 闹钟** | 表格 + 周视图；**一句话添加**（跟语音说的是同一套解析：「明天下午三点组会，提前半小时」）；编辑/删除/跳过本次/标记完成；重复事件会先确认 |
| **备忘** | 一句话记下、勾掉、删除、清掉已完成（清洗规则和语音一致：「记一下买牛奶」存的就是「买牛奶」） |
| **日志** | 实时跟随 `sessions/listen.log`（打开页面就有最近几百行），报错标红、警告标黄，可暂停跟随/只看报错 |
| **角色 / 声线** | 每个角色的人设要点、唤醒词、参考音与专属模型目录（文件在不在如实标出）；**切过去**、**切过去并试听** |
| **文字对话** | 在网页里打字，走**完整链路**（技能/工具 → LLM → 说出来）：发一句话，她回答并念出来；**可以选角色**（用谁的设定与声线答，选项里会标出「现在是她」）；也能「只念不答」当试听用 |

**它是独立进程**，这点很重要：

- 界面崩了不影响语音服务；改控制台代码不用重启语音服务
- 它**不碰麦克风、不加载模型**，所以内存很小；要出声一律让服务去做
- 改日程/闹钟是**直接读写同一批 json**（`data/events.json`、`data/memos.json`）；
  服务每 5 秒轮询一次到期事件，用的就是同一个文件，**所以网页里改完不用重启服务，5 秒内就生效**
- 「念一句 / 切声线 / 文字对话」这类只有服务能做的事，走一个**文件信箱**
  （`sessions/console/` 下的 `inbox/` `busy/` `done/`）：控制台投请求，服务执行完写回执。
  你甚至可以自己进去看 json，一看就知道控制台刚才让服务干了什么
- 服务没在跑时这些按钮会**立刻**告诉你「先点右上角启动服务」，不会让你干等超时
- **启动/停止都会验证结果**：点「启动」会等它真的起来（最多 25 秒）才说成功，
  没起来就把**日志最后一行**一起告诉你（比如缺模型）；有上次留下的 pid/停止信号文件会**自动清掉**，
...
> 去沙沙声用 `out_tilt_hz` / `out_tilt_db`（默认 **10 kHz 以上 −9 dB**：实测沙在 10-12 kHz 那层，
...
> 在 5 遍配对里反而让输出更亮。详见工程日志第 18 节。
>
> **⚠️ 2026-09-25 更正（去沙沙声：指纹在 10~12 kHz）**：第三轮量出沙沙声长什么样——
> 自然语音的高频**一路滚降**（真人参考：4-6k −10.7 → 6-8k −11.3 → 8-10k −13.6 → 10-12k **−16.0** dB），
> 而微调模型的 **10-12k 是 −8.0 dB**（比自己的 8-10k 还高 4.3 dB）——就是耳朵听到的沙。
> 它**不在停顿里**（ZipVoice 的停顿是精确数字零），所以参考净化/噪声门都没用（那就是 18.2 无效的原因）；
> 旧的 `out_tilt 7kHz/−3` **打偏了**：把 8-10k 砍了 3 dB（齿音变闷）而 10-12k 只降 3 dB（依旧翘着）。
> 现在默认 **`out_tilt_hz = 10000` / `out_tilt_db = -9`**：4-8k 一点不动、10-12k 落到 −22 dB。
> ★这个值跟模型绑定★（微调 −9 / 出厂 −4，出厂模型本来就不沙）；换模型/换参考后先量：
> `python scripts/ab_clone_model.py --profile <wav...>`。试听 `sessions/hiss_last/`。详见工程日志第 20 节。
>
...
> 详见工程日志第 19 节。
>
> **要不要加数据重训？实测：不用**（日志 19.6）。出厂模型（Emilia 大数据）与 38 条/4.8 分钟的
> 微调版，抖动是同一量级（0.89 vs 1.30 半音，差别在噪声里）——因为抖动来自「每次 generate 都
> 重新采样」，不是数据少。**能撬动它的是参考音频（同一模型换参考能差近 4 倍：0.90~3.41 半音），
> 但关键不是把参考加长**（16 s 的反而 3.09）。顺带发现 `精英化晋升2.wav` 这类情绪化参考会让
> 输出几乎没有浊音段（5 遍里 4 遍量不出音高），要避开。真要继续压，就去挑参考（免费），别去训。
...
| 层 | 人话 | 存在哪 | 会不会忘 |
| --- | --- | --- | --- |
| **L1 原始对话** | 刚说过的话 | `sessions/session-*.jsonl` | 会：滑窗保留最近 **30 天 / 10 个文件**（已提取入库的才敢删） |
| **L2 情节** | 「记得那件事」 | `data/memory/<角色>/episodes.jsonl` | 会：显著性按 **60 天半衰期**衰减；**被问起一次就加回来一点** |
| **L3 事实** | 人设 / 习惯 / 长期偏好 | `data/memory/<角色>/facts.json` | 一般会；**名字、称呼这类 `pinned` 永不衰减** |
| **L4 知识** | 你给它的背景资料 | `data/knowledge/*.md`、`config.toml` 的 `world` | 不会，你放什么它读什么 |

### 用法

平时不用管 —— 每轮对话它会自动挑最相关的几条记忆拼进提示。想自己查：

```powershell
python main.py memory                      # 四层各有多少
python main.py memory --recall "导师"        # 信息检索
python main.py memory --recall "导师" --when 上周   # ★时间 + 信息★：「上周三我们聊过什么」
python main.py memory --remember "记住：周五三点见导师"
python main.py memory --consolidate        # 手动跑一遍自清洁（合并/衰减/提升）
python main.py memory --prune --apply      # 真删已入库的旧原始对话
python main.py memory --who baize --forget <id>
```

时间词支持「昨天 / 今天 / 前天 / 上周 / 上周三 / 最近 7 天 / 上个月 / 去年 / 2026-09-20」；
**认不出来它就不按时间筛（不猜）** —— 猜错时间比不筛更坏。

### 自清洁（什么时候自己动手）

- **合并**：36 小时内、话题重合的两条情节并成一条；
- **衰减**：60 天半衰期，被检索命中就加回 * 一次；
- **提升**：同一件事在 **≥ 3 个不同日子**出现过 → 升为事实（「他每周三有课」）；
- **裁剪**：情节 > 800 条 / 事实 > 200 条就按权重砍尾。

真正的触发时机是**每次优雅退出**（`close()` 里先归档、再整理）；平时不打扰你。

### 换世界观 / 接知识库

把 `.md` / `.txt` / `.json` 丢进 `data/knowledge/` 就行（格式看里面的 `README.md`），
或者临时写在 `config.toml`：

```toml
[memory]
enabled = true
world = "白泽是通晓万物之名的瑞兽，如今住在本地机器里……"   # 临时一段设定
```
...
| `data/knowledge/<角色id>/任何文件.md` | **只有那个角色** |
| `data/knowledge/_worlds/<世界观>/…` | **挂了这个世界观的角色**（同 IP 共享一套） |

```
data/knowledge/
    _README.md            ← 下划线开头 = 说明文件，不当知识
    通用说明.md            ← 大家都看
    baize/世界观.md        ← 只有白泽看
    kaltsit/补注.md        ← 只有凯尔希看
    _worlds/明日方舟/泰拉.md   ← ★凯尔希 + 阿米娅都看★（一份文件，两人共用）
```
...
两个明日方舟的角色共享 `data/knowledge/_worlds/明日方舟/` —— **想改设定只改一处**。
白泽没挂任何世界观，所以它拿不到明日方舟的设定（它有自己的 `baize/`）。

一个角色的知识库是三层叠起来的（以凯尔希为例）：

```
共享（所有人） → 世界观组（挂了的人） → 专属（自己） → 内联 world（人格文件里写的）
```

可挂多个世界观（`["明日方舟", "联动活动"]`）；`knowledge_shared = false` 只关掉**共享那一层**，
世界观组是它自己挂的，照用（否则凯尔希会说「我不是明日方舟的」）。
看 `stats` 就知道每层各有多少段：`{'local': 6, 'local:world:明日方舟': 4, 'local:kaltsit': 2}`。
...
```json
{ "memory_all": true,      // 能查所有人的记忆（查完会说清每条是谁记的）
  "knowledge_all": true }  // 能看全部知识库（所有世界观 + 别人的专属资料）
```

- **`memory_all`**：`python main.py memory --recall "体检报告" --all` 就跨角色查；
  对话里模型自己用 `who="*"`（或「全部角色」）—— ★每条结果都标着属于谁★。
- **`knowledge_all`**：它的知识库里会**额外并入**所有 `_worlds/<世界观>/` 和别人的专属资料 ——
  于是它既能聊明日方舟、也能聊别人的设定；**看得到归看得到，每条片段都带着归属**
  （检索结果里会显示 `[资料·凯尔希 的专属资料]`、`[资料·明日方舟（世界观）]`）。
- ★**不代入**★：开了这两个权限的角色，系统提示里会自动多一段要求 ——
  提到别人时用旁观说法（「据我所知」「在……的设定里」），**不要说成自己的身份、来历或经历**。
  没有这一段，小模型会把资料当成自己（「我来自泰拉」「我是罗德岛的医疗主管」），而且不会报错。

**特意关掉**：`"knowledge_all": false` → 还能跨角色查记忆，但不再看别人的世界观；
`"memory_all": false` → 连跨角色查记忆也关掉（两个都关就是严格只看自己那份）。
不写 `knowledge_all` 时它**跟随 `memory_all`** —— 这就是「默认拥有、除非特意关闭」的写法。

别的角色默认没有这两个权限（`false`），所以凯尔希看不见阿米娅的、也看不见白泽的；
找不到该角色时也一律**不给**权限（不会退回默认角色 —— 白泽恰好开着，一退就是"不小心把权限发出去了"）。
命令行加 `--all` 不需要权限：那是你本人在终端上查，权限拦的是**角色**。
- 每轮自动拼进提示词的那份摘要**不跨角色**（只带当前角色），跨角色只发生在你明着问的时候。

### 多角色：记忆分开，日程共用

- 每个角色一份目录 `data/memory/<角色>/`：**经历、事实、知识互不可见**；
- 唯一共享的是**日程**：记忆只**读** `data/events.json`，写入永远只走技能 —
  两条写路径必然互相覆盖，所以一开始就只留一条。

### 能删吗

能。删文件就等于失忆：删 `episodes.jsonl` 丢经历、删 `facts.json` 连人设也丢
（重开一次会重新种下名字 / 称呼这类身份事实）；旧原始对话怕删错就先
`python main.py memory --prune`（默认只看不删）。

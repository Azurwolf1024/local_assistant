...
### 白泽可以查全部人的记忆（★但要说明是谁记的★）

默认助手白泽在人格文件里开了 `"memory_all": true`，所以它能跨角色查：

```powershell
python main.py memory --recall "体检报告" --all     # 命令行：查所有人，每条标出属于谁
```

对话里模型自己也会用：`mcp__memory__recall` 加上 `who="*"`（或「全部角色」）——
★回答时必须说出每条是哪个角色记的★（工具返回的每一行都带 `[白泽]` / `[凯尔希]` 这样的前缀）。

别的角色没这个权限，试了会被直接拒：

> 凯尔希没有查所有人记忆的权限 —— 只能查自己的。需要的话在人格文件里加 `"memory_all": true`。

几个细节：

- 权限写在**人格文件**里（默认 `false`）；找不到这个角色时一律**不给**权限
  （不会退回默认角色 —— 白泽恰好开着，一退就是“不小心把权限发出去了”）。

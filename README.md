...
| 一次性的日程 | 明天下午三点安排组会 |
| 改日程 | 把组会挪到周五上午十点 / AIAA3102 改成下午三点 |
...
- `skip`：**只跳过这几天**的日期数组（如 `["2026-09-23"]`），课表本身留着；
  说「下周三的课不上了」就是往这里加一天，改时间时会自动清空
...
- 改：「把组会挪到周五上午十点」（换星期）、「AIAA3102 改成下午三点」（只换时刻）
- 只取消这一次：「下周三的课不上了」「这周三的课取消」
- 彻底删除：「以后不上这门课了」「删掉组会」

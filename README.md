...
一句话：**模型**负责「听懂要干什么」，**确定性代码**负责「算时间、查数据、做守卫」。
...
| | 结论 |
| --- | --- |
| 正则先行的收益 | 62% 的请求 **0 ms 决策**；交给模型的 38% 里绝大多数是闲聊 |
| 正则先行的代价 | 真实对话里只有 **1 轮**漏网（就是「跟导师见面」那次），但它花 10.4 s 编了句「已记录此安排」 |
| 模型会不会选工具 | **会**：口语/ASR 变体 8/8 都选对了（含「下礼拜三下午三点半碰个面」） |
| 模型能不能算日期 | **不能**：「下周三下午三点半」算成 10-04，「每周四」把 repeat 丢了、地点也丢 |
...
| 一次性 | 「明天下午三点安排组会」「帮我记录晚上七点半有跆拳道课」 || 约会 | 「下周三下午三点半跟导师见面」「明天下午三点有个面试」「后天上午体检」「大后天中午和导师吃饭」 || 每周 / 每两周 | 「每周三上午九点有 AIAA3102 机器学习，地点教学楼 A302」「每两周周三下午两点开组会」 |

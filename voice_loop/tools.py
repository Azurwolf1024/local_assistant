...
为什么是这个形状（2026-09-18 实测之后定的）
    - 本机 qwen2.5:7b **会选工具**（7/7 条口语命令都选对了，包括「下礼拜三」这种 ASR 口吻），
      但**算日期很不可靠**（「下周三下午三点半」算成 10-04；「每周四」把 repeat 丢了）。
...
TOOL_HINT = (
    "你可以调用工具。规则：\n"
    "1. 用户问到日程/课程/会议/提醒/备忘的**事实**（有什么、几点、下次是什么），先调工具查，"
    "不要凭记忆或习惯回答——你的记忆里没有他的日程。\n"
    "2. 用户要**记东西或排日程**时，把他的话**原样**放进 text 参数，"
    "不要自己换算日期、不要改写时间说法（「下周三下午三点半」就照抄）。\n"
    "3. 闲聊、常识、提问不涉及上述内容时，直接回答，不要调工具。\n"
    "4. 工具返回的 reply 就是最终答复，不要改数字和时间的说法。\n"
    "5. 要调工具就**直接用工具接口**，不要用文字写出 {'name': ..., 'arguments': ...} 这种 JSON。"
)
